...
```bash
cp .env.example .env        # fill in GITHUB_TOKEN and NVIDIA_API_KEY
uv sync
uv run uvicorn app.main:app --reload
```

`GITHUB_TOKEN` just needs `public_repo`/read scopes — it's only reading public data.
`NVIDIA_API_KEY` comes from https://build.nvidia.com (free tier).
...
- **`ai_service`** returns structured `tips` (issue / action / impact) instead of a
  single prose string, and uses `AsyncOpenAI` so the LLM call no longer blocks the
  FastAPI event loop.
...
- `profile_cache` is in-process memory (`TTLCache`). Fine for one instance; once you
  run multiple workers/replicas, swap `app/core/cache.py` for a Redis-backed version —
...
- No per-user rate limiting / tiering (e.g. "free users get math score only"). That's
  a product decision this service doesn't make on its own — likely belongs in your
  API gateway or a thin auth layer in front of this service.
- README matching only tries `README.md` and `readme.md` at the repo root — it will
  miss READMEs with other casings/paths. Not worth over-engineering until you see it
  matter in real data.
